...
close). The UI surfaces them in steps 8-9 (recents window + stats
window).
...
def _read_from_disk() -> dict:
    ...
    # Ensure top-level shape exists, filling in missing keys from defaults.
...
def record_tick(file_path: str, tick_seconds: float) -> None:
    ...
    pf = cache["per_file"].setdefault(file_path, {
        "tokens_read": 0,
        "seconds_active": 0.0,
        "last_opened": "",
    })
